...
def run_momentum_engine():
    ...
    for symbol in symbols:
        ...
        try:
            ...
            #=======================+++++++++++++++++++++++++++++++
            current_month_df = df_reset[
            df_reset["date"].dt.to_period("M") == pd.to_datetime(LAST_TRADING_DATE).to_period("M")
            ]

            if len(current_month_df) > 0:
                current_month_candle = {
                    "date": pd.to_datetime(LAST_TRADING_DATE),
            ...
            monthly = monthly[monthly["date"] <= pd.to_datetime(LAST_TRADING_DATE)]
        ...
        weekly_ema = calculate_ema_tv(weekly["close"], 9)
        ...
        monthly_ema = calculate_ema_tv(monthly["close"], 9)
        monthly_ema9 = float(monthly_ema.iloc[idx])

        rsi_daily = calculate_rsi_tv(df["close"], 10)

        current_month = pd.to_datetime(LAST_TRADING_DATE).to_period("M")
        #+++++++++++++++++++++++++++++++++++++++++
        monthly_rsi_series = []

        for m_date in monthly["date"]:
            subset = df[df["date"] <= m_date]
            if len(subset) >= 10:
                val = calculate_rsi_tv(subset["close"], 10).iloc[-1]
            else:
                val = np.nan
            monthly_rsi_series.append(val)

        monthly["rsi10"] = monthly_rsi_series

        monthly_rsi10_t1 = float(monthly["rsi10"].iloc[idx])
